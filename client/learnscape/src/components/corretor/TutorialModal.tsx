...
const TutorialModal: React.FC<TutorialModalProps> = ({ isOpen, steps, onClose }) => {
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      {/* Blur de fundo */}
      {isOpen && <div className="fixed inset-0 bg-black opacity-30" onClick={onClose}></div>}

      <div className="bg-white dark:bg-gray-800 p-4 rounded-md shadow-md transition-opacity duration-300 relative">
        ...
        <div className="flex justify-between">
          <button
            className={`px-4 py-2 bg-blue-500 text-white rounded ${currentStep === 0 ? 'invisible' : ''}`}
